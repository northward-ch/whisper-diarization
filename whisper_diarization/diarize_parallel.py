...
def diarize_parallel(vocal_target: Path, txt_path: Path, srt_path: Path, tmp_dir: Path, suppress_numerals: bool, model_name: str, batch_size: int, language: str, device: str):
    logging.info("Starting Nemo process with vocal_target: ", vocal_target)
    nemo_process = subprocess.Popen(
        ["python", "nemo_process.py", "-a", vocal_target, "--device", device],
        stderr=subprocess.PIPE,
    )

    whisper_model = faster_whisper.WhisperModel(
        model_name, device=device, compute_type=helpers.mtypes[device]
    )
    ...
    assert nemo_return_code == 0, (
        f"Diarization failed: {nemo_error_trace.decode('utf-8')}"
    )
    ...
    with open(txt_path, "w", encoding="utf-8-sig") as ftxt:
        helpers.get_speaker_aware_transcript(ssm, ftxt)

    with open(srt_path, "w", encoding="utf-8-sig") as fsrt:
        helpers.write_srt(ssm, fsrt)
